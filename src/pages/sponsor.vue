  ...
  <div :class="isPC ? 'pc_box' : 'mobile_box'">
    ...
    <div class="page_content travel">
      <div class="trave_left">
        <div class="text_box">
          <p class="title">{{$t("sponsor.title")}}</p>
          <p class="fonts mt30">{{$t("sponsor.second")}}</p>
        </div>
        <div class="center_box">
          <div class="bth_pdf">
            <div class="tips">{{$t("sponsor.tips")}}</div>
            <a href="static/Become_sponsor.pdf" class="block" target="_blank" v-if="isPC"></a>
            <a href="static/Become_sponsor.pdf" class="block" download="Become _sponsor.pdf" v-else></a>
          </div>
        </div>    
      </div>
      <!-- 广告列表 -->
